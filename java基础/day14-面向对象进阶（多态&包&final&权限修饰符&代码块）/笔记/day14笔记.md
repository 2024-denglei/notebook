...
调用成员变量时：编译看左边，运行看左边
javac在编译代码的时候，会看左边的父类有没有这个变量，如果有编译成功，没有就失败
java运行代码的时候，实际获取的就是左边父类中成员变量的值

调用成员方法时：编译看左边，运行看右边
javac在编译代码的时候，会看左边的父类有没有这个方法，如果有编译成功，没有就失败
java运行代码的时候，实际运行的是右边子类的方法
...
f.show();
```
![](assets/day14笔记/file-20251118195231816.png)
## 1.5 多态的弊端
